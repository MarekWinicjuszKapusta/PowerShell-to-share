...
function removeLeftOvers{
    ...
    if(Test-Path $Path){
        ...
        if(Test-Path $Path){
            LogWrite "error: complications with deleting $path, will try again after 10 seconds"
            $attemp = 2
...
if(test-path "C:\Program Files (x86)\Teams Installer\Teams.exe"){
    ...
    foreach($user in $ActiveExplorer){
        ...
        elseif(Test-Path "C:\Program Files (x86)\Microsoft\Teams\update.exe"){
            LogWrite "Teams folder with update.exe found for user in C:\Program Files (x86)\Microsoft\Teams\, uninstalling MS Teams..."
            Start-Process -FilePath "C:\Program Files (x86)\Microsoft\Teams\update.exe" -ArgumentList "-uninstall -s" -EA Stop
            removeLeftOvers -path  "C:\Program Files (x86)\Microsoft\Teams"
            removeLeftOvers -path "C:\Program Files (x86)\Microsoft\teamsMeetingAddin"
            removeLeftOvers -path "C:\Program Files (x86)\Microsoft\teamsPresenceAddin"
            removeLeftOvers -path "C:\Users\$username\AppData\Roaming\Microsoft\teams"
            removeLeftOvers -path "C:\Users\$username\AppData\Roaming\teams"
            removeLeftOvers -path "C:\Users\$username\AppData\local\Microsoft\teams"
            removeLeftOvers -path "C:\Users\$username\AppData\local\Microsoft\teamsMeetingAddin"
    ...
    LogWrite "Before running script, please install MS Teams machine wide installer x64"
}
